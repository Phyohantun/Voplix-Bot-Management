...
function relOrder(iso: string | null) {
  if (!iso) return 'No orders yet';
  const diff = Date.now() - new Date(iso).getTime();
  const m = Math.floor(diff / 60000);
  if (m < 1) return 'Last order just now';
  if (m < 60) return `Last order ${m}m ago`;
  const h = Math.floor(m / 60);
  if (h < 48) return `Last order ${h}h ago`;
  const d = Math.floor(h / 24);
  return `Last order ${d}d ago`;
}
  ...
  return (
    <>
      ...
      >
        ...
        >
          {healthy ? 'Webhook connected — receiving messages' : 'Webhook needs setup — tap Reconnect'}
        </div>
        <CardHeader className="pb-3">
          ...
          <div className="mt-3 grid gap-1.5 text-xs text-zinc-600 dark:text-zinc-400">
            ...
            <p className="text-zinc-500">{relOrder(stats.lastOrderAt)}</p>
          </div>
        </CardHeader>
      ...
      <Dialog open={statsOpen} onOpenChange={setStatsOpen}>
        <DialogContent className="max-w-md border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
          <DialogHeader>
            ...
            <DialogDescription className="text-zinc-600 dark:text-zinc-400">Shop performance snapshot</DialogDescription>
          </DialogHeader>
          <ul className="space-y-2 text-sm text-zinc-800 dark:text-zinc-200">
            ...
            <li className="flex justify-between gap-4">
              <span className="text-zinc-500">Revenue (completed)</span>
